...
class I3D_model():
  def __init__(self, modality='RGB', num_classes=101):
    ''' modality is one of ['RGB', 'FLOW] '''
    self.modality = modality
    self.num_classes = num_classes

  def inference(self, input_image, is_training, end_point='Logits'):
    dropout_keep = tf.cond(is_training, lambda: 0.2, lambda: 1.0)

    if self.modality == 'RGB':
      return self.rgb_inference(input_image, is_training, dropout_keep, end_point)
  ...
  def rgb_inference(self, input_images, is_training, dropout_keep, end_point='Logits'):
    ...
    with tf.variable_scope('I3D_Model'):
      ...
      rgb_logits, end_points = rgb_model(
          rgb_input, is_training=is_training, dropout_keep_prob=dropout_keep)
  ...
  def flow_inference(self, input_images, is_training, dropout_keep, end_point='Logits'):
    ...
    with tf.variable_scope('Flow'):
      flow_model = InceptionI3d(
          self.num_classes, spatial_squeeze=True, final_endpoint=end_point)
      flow_logits, end_points = flow_model(
          flow_input, is_training=is_training, dropout_keep_prob=dropout_keep)
